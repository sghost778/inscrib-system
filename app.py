    ...
    with app.app_context():
        ...
        try:
            ...
            if col_type and col_type.upper() != "INTEGER":
                ...
                db.session.commit()
                print("[OK] Migración: REGISTRO_AUDITORIA.id_log corregida a INTEGER AUTOINCREMENT")
        except Exception as mig_err:
            db.session.rollback()
            print(f"[INFO] Migración de auditoría omitida: {mig_err}")
        ...
        try:
            ...
            if AnoEscolar.query.count() == 0:
                ...
                db.session.commit()
                print("Datos de prueba: Año y Grados inyectados.")
                ...
                if ciudad:
                    ...
                    if rep and ano and grado1 and grado2:
                        try:
                            ...
                            print("Datos de prueba: Estudiantes inscritos (Matrícula inyectada).")
        ...
        try:
            if Noticia.query.count() == 0:
                for nd in [
                    {"titulo": "Inicio de Clases 2026-2027", "resumen": "Las inscripciones para el nuevo año escolar están abiertas.", "contenido": "Período de inscripciones abierto.", "imagen": "/static/uploads/noticia1.jpg"},
                    {"titulo": "Jornada Deportiva Anual", "resumen": "Jornada deportiva con participación de todos los niveles.", "contenido": "Jornada Deportiva Anual 2026 realizada con éxito.", "imagen": "/static/uploads/noticia2.jpg"},
            ...
            if ProgramaAcademico.query.count() == 0:
                for pd in [
                    {"nombre": "Educación Inicial", "descripcion": "Programa para niños de 3 a 5 años.", "nivel": "Inicial", "icono": "fa-child"},
                    {"nombre": "Educación Primaria", "descripcion": "Formación integral de 1ero a 6to grado.", "nivel": "Primaria", "icono": "fa-book-open"},
                    {"nombre": "Educación Media General", "descripcion": "Bachillerato general de 1ero a 5to año.", "nivel": "Bachillerato", "icono": "fa-graduation-cap"},
                    {"nombre": "Educación Especial", "descripcion": "Atención educativa integral.", "nivel": "Especial", "icono": "fa-hands-helping"},
                    {"nombre": "Formación Docente", "descripcion": "Actualización y formación continua.", "nivel": "Docente", "icono": "fa-chalkboard-teacher"},
                ]:
                    db.session.add(ProgramaAcademico(**pd))
                db.session.commit()
                print("Datos de prueba: Programas Académicos inyectados.")

            if Galeria.query.count() == 0:
                for gd in [
                    {"titulo": "Instalaciones Deportivas", "imagen": "/static/uploads/galeria1.jpg"},
                    {"titulo": "Salón de Clases", "imagen": "/static/uploads/galeria2.jpg"},
                    ...
                    {"titulo": "Área de Recreación", "imagen": "/static/uploads/galeria2.jpg"},
                    {"titulo": "Auditorio", "imagen": "/static/uploads/galeria3.jpg"},
                ]:
                    db.session.add(Galeria(**gd))
                db.session.commit()
                print("Datos de prueba: Galería inyectada.")

            if SiteConfig.query.count() == 0:
                for k, v in [
                    ("site_name", "U.E. Dr. Jose Manuel Cova Maza"),
                    ("site_description", "Formando líderes para el futuro con excelencia educativa"),
                    ("about_title", "Quiénes Somos?"),
                    ("about_content", "Institución educativa comprometida con la formación integral."),
                    ("about_mision", "Formar ciudadanos integrales con valores éticos."),
                    ("about_vision", "Ser institución de referencia nacional."),
                    ("contact_address", "Av. Principal, Puerto Ordaz, Estado Bolívar"),
                ...
                db.session.commit()
                print("Datos de prueba: Configuración del sitio inyectada.")
